$(document).ready(function () {
    $("#formularios").empty();
    $.get("php/vehiculosDisponibles.php", function(respuesta) {
...
function creaCards(xml) {
    ...
    xml.querySelectorAll("vehiculo").forEach(elem => {
        ...
        img.setAttribute("src", "/TFG/Codigo/imgVehiculos/"+elem.querySelector("imagen").textContent);
        card.append(img);
...
function negociar(oEvento) {
    ...
    $.post("php/negociarVehiculo.php", ("id="+id), function(respuesta) {
        alert(respuesta.mensaje);
        if (respuesta.error == 0) {
            $("#formularios").empty();
            window.location = "misChats.html";
        }
    }, "json");
}