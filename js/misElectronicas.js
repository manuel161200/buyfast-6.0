$(document).ready(function() {
    $("#formularios").empty();
    $.get("php/misElectronicas.php", function(respuesta) {
...
function creaCards(xml) {
    ...
    xml.querySelectorAll("electronica").forEach(elem => {
        let card = document.createElement("div");
        card.classList.add("card");
        let img = document.createElement("img");
        img.classList.add("card-img-top");
        img.setAttribute("src", "/TFG/Codigo/imgElectronicas/"+elem.querySelector("imagen").textContent);
        card.append(img);
        ...
        modelo.append(elem.querySelector("modeo"));
        lista.append(modelo);
        let color = document.createElement("li");
        color.classList.add("list-group-item");
        color.append("Tipo: ");
        color.append(elem.querySelector("tipo"));
        lista.append(color);
...
function vendido(oEvento) {
    ...
    $.post("php/venderElectronica.php", ("id="+id), function(respuesta) {
...
function disponible(oEvento) {
    let oE = oEvento || window.event;
    let id = oE.currentTarget.dataset.id_producto;
    $.post("php/disponibleElectronica.php", ("id="+id), function(respuesta) {
        alert(respuesta.mensaje);
        if (respuesta.error == 0) {
            location.reload();
        }
